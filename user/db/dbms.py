...
def edit_doctor_profile(conn,medical_code,username,first_name,last_name,password,visit_price,specialty_name):
    ...
    conn.execute(sql,[username,first_name,last_name,password,visit_price,specialty_id,medical_code])
    print("edit done")
    return True

def doctor_add_insurance(conn,medical_council_code,insurance_name):
    insurance_id = get_insurance_id(conn,insurance_name)
    doctor_user_name =get_doctor_name(conn,medical_council_code)[2]
    sql = "insert into in_contract_with values (?,?,?) "
    cursor =conn.execute(sql,[insurance_id,medical_council_code,doctor_user_name])
    return "insurance " + insurance_name +" added succesfully for "+ doctor_user_name


def doctor_delete_insurance(conn,medical_council_code,insurance_name):
    insurance_id = get_insurance_id(conn, insurance_name)
    doctor_user_name = get_doctor_name(conn, medical_council_code)[2]
    sql = "delete from in_contract_with where insurance_id = ?  and medical_council_code = ?  "
    cursor = conn.execute(sql, [insurance_id, medical_council_code])
    return "insurance " + insurance_name+ " deleted succesfully for " + doctor_user_name



def doctor_edit_work_hour(conn,medical_council_code,start_hour,end_hour,place,wh_year,wh_month,wh_day):
    if place=='doh':
        sql ="update work_hour set start_hour =?,end_hour =? ,wh_year =?,wh_month =?,wh_day =? where work_hour.doh_id in(select doh_id from doh where medical_council_code =?)"
    else :
        sql = "update work_hour set start_hour =?,end_hour =? ,wh_year =?,wh_month =?,wh_day =? where work_hour.dhh_id in(select dhh_id from dhh where medical_council_code =?)"
    cursor = conn.execute(sql,[start_hour,end_hour,wh_year,wh_month,wh_day,medical_council_code])
    return "edit hour succesfully"
...
def search_doctors_based_on_insurance(conn, insurance):
    insurance_id = get_insurance_id(conn, insurance)
    sql1 = "select doctor.medical_council_code, doctor.first_name, doctor.last_name, doctor.visit_price, specialty.name from doctor, specialty where doctor.specialty_id = specialty.specialty_id and doctor.medical_council_code in (select medical_council_code from in_contract_with where insurance_id = ?)"
    cursor = conn.execute(sql1, [insurance_id])
    data = cursor.fetchall()
    res = "medical_code |  first_name |  last_name | visit_price |  specialty \n\n"
    for row in data:
        res += row[0] + " " + row[1] + " " + row[2] + " " + str(row[3]) + " " + row[4] + '\n'
    return res



def docotrs_of_a_medical_center(conn, health_care_center_phone_number):
    sql = "select doctor.medical_council_code, doctor.first_name, doctor.last_name, doctor.visit_price, specialty.name from doctor, specialty where medical_council_code in( select medical_council_code from dhh where health_care_center_id in" \
          "(select health_care_center_id from health_care_center where phone_number =? ))"
    cursor = conn.execute(sql, [health_care_center_phone_number])
    data = cursor.fetchall()
    res = "medical_code |  first_name |  last_name | visit_price |  specialty \n\n"
...
# prints those who works in a health care center
def docotrs_of_all_medical_centers(conn):
    sql = "select doctor.medical_council_code, doctor.first_name, doctor.last_name, doctor.visit_price, specialty.name from doctor, specialty where specialty.specialty_id = doctor.specialty_id and medical_council_code in(select medical_council_code from dhh ) "
    cursor = conn.execute(sql)
    data = cursor.fetchall()
    res = "medical_code |  first_name |  last_name | visit_price |  specialty \n\n"
    for row in data:
...
# prints all doctors informations
def show_all_doctors(conn):
    sql = "select doctor.medical_council_code, doctor.first_name, doctor.last_name, doctor.visit_price, specialty.name from doctor, specialty where specialty.specialty_id = doctor.specialty_id"
    cursor = conn.execute(sql)
    data = cursor.fetchall()
    res = "medical_code |  first_name |  last_name | visit_price |  specialty \n\n"
    for row in data:
